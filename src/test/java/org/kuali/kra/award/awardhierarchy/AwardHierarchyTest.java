...
public class AwardHierarchyTest {
    ...
    public void testFindingAssociatedAward_PendingAndActiveAwardsExist() {
        ...
        awards.add(award);
        VersionHistory versionHistory = new VersionHistory(award, VersionStatus.ACTIVE, PersonFixture.UAR_TEST_001.getPrincipalName(), new Date(System.currentTimeMillis()));
        ...
        awards.add(award);
        versionHistory = new VersionHistory(award, VersionStatus.PENDING, PersonFixture.UAR_TEST_001.getPrincipalName(), new Date(System.currentTimeMillis()));
        versionHistories.add(versionHistory);
    ...
    private AwardHierarchy prepareSingleNode(VersionStatus status, Long awardId) {
        ...
        award.setAwardNumber(generateAwardNumber(BASE_AWARD_NUMBER, BASE_HIERACHY_SEQUENCE));
        VersionHistory versionHistory = new VersionHistory(award, status, PersonFixture.UAR_TEST_001.getPrincipalName(), new Date(System.currentTimeMillis()));
        AwardHierarchy node = new AwardHierarchy(award.getAwardNumber(), AwardHierarchy.ROOTS_PARENT_AWARD_NUMBER, award.getAwardNumber());
        node.setAward(null);
