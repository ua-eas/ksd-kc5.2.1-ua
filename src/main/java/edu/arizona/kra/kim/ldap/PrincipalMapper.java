 ...
 */
package edu.arizona.kra.kim.ldap;
...
public class PrincipalMapper extends BaseMapper<Principal> {
    ...
    Principal.Builder mapBuilderFromContext(DirContextOperations context) {
    	...
    	// aka, uaid, of the form "123456789012" (12 digits), EDS REQUIRED
        final String entityId      = context.getStringAttribute(getConstants().getKimLdapIdProperty());
        
        // aka uid, aka NetID, e.g.: "sskinner", EDS REQUIRED
        String principalName = context.getStringAttribute(getConstants().getKimLdapNameProperty());
        
        if(StringUtils.isBlank(principalName)){
        	// Principal.Builder will craok if principalName is empty 
        	principalName = entityId;
        }
