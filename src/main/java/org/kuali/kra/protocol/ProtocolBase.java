...
public abstract class ProtocolBase extends KraPersistableBusinessObjectBase
		implements SequenceOwner<ProtocolBase>, Permissionable,
		UnitAclLoadable, Disclosurable, KcKrmsContextBo {
	...
	// TODO: This should be refactored when IRB and IACUC are merged. For now,
	// demote into IACUC
	// private List<ProtocolCustomData> personCustomDataList = new
	// AutoPopulatingList<PersonCustomData>(PersonCustomData.class);
	...
	/*
	 * There should only be zero or one entry in the protocolAmendRenewals list.
	 * It is because of OJB that a list is used instead of a single item.
	 */
	...
	public Date getSubmissionDate() {
		...
		// getProtocolSubmission will get the last one. SO, this method may not
		// needed.
	...
	public void setProtocolResearchAreas(
	...
	public void addProtocolResearchAreas(
			ProtocolResearchAreaBase protocolResearchArea) {
	...
	public ProtocolResearchAreaBase getProtocolResearchAreas(int index) {
		while (getProtocolResearchAreas().size() <= index) {
			getProtocolResearchAreas()
					.add(getNewProtocolResearchAreaInstance());
		}
	...
	public void setProtocolReferences(
			List<ProtocolReferenceBase> protocolReferences) {
	...
	public void setProtocolLocations(
			List<ProtocolLocationBase> protocolLocations) {
	...
	public List buildListOfDeletionAwareLists() {
		...
		if (getProtocolAmendRenewal() != null) {
			...
			// needed to ensure that the OjbCollectionHelper receives constant
			// list size.
		...
		for (ProtocolSpecialReviewBase specialReview : getSpecialReviews()) {
			specialReviewExemptions.addAll(specialReview
	...
	private List<ProtocolAttachmentPersonnelBase> getProtocolAttachmentPersonnel() {
		...
		for (ProtocolPersonBase protocolPerson : getProtocolPersons()) {
			protocolAttachmentPersonnel.addAll(protocolPerson
					.getAttachmentPersonnels());
		}
	...
	public ProtocolPersonBase getPrincipalInvestigator() {
		return getProtocolPersonnelService().getPrincipalInvestigator(
				getProtocolPersons());
	}
	...
	public ProtocolUnitBase getLeadUnit() {
		...
		if (getPrincipalInvestigator() != null) {
			for (ProtocolUnitBase unit : getPrincipalInvestigator()
					.getProtocolUnits()) {
		...
		if (StringUtils.isBlank(principalInvestigatorId)) {
			if (getPrincipalInvestigator() != null) {
				...
				if (StringUtils.isNotBlank(principalInvestigator.getPersonId())) {
					setPrincipalInvestigatorId(principalInvestigator
							.getPersonId());
				} else {
	...
	protected ProtocolLocationService getProtocolLocationService() {
		ProtocolLocationService protocolLocationService = (ProtocolLocationService) KraServiceLocator
	...
	public void setProtocolFundingSources(
	 ...
	 * 
	 * @param attachmentProtocol
	 *            the attachment protocol
	 * @throws IllegalArgumentException
	 *             if attachmentProtocol is null
	 */
	private void addAttachmentProtocol(
			ProtocolAttachmentProtocolBase attachmentProtocol) {
		ProtocolAttachmentBase.addAttachmentToCollection(attachmentProtocol,
				this.getAttachmentProtocols());
	}
	 ...
	 *             if attachmentProtocol is null
	 */
	private void removeAttachmentProtocol(
			ProtocolAttachmentProtocolBase attachmentProtocol) {
		ProtocolAttachmentBase.removeAttachmentFromCollection(
				attachmentProtocol, this.getAttachmentProtocols());
	}
	 ...
	 *            the attachment
	 * @throws IllegalArgumentException
	 *             if attachment is null or if an unsupported attachment is
	 *             found
	 */
	public <T extends ProtocolAttachmentBase> void addAttachmentsByType(
			T attachment) {
		if (attachment == null) {
	 ...
	 *             found
	 */
	public <T extends ProtocolAttachmentBase> void removeAttachmentsByType(
			T attachment) {
		if (attachment == null) {
			throw new IllegalArgumentException("the attachment is null");
		}
	...
	public ProtocolSubmissionBase getProtocolSubmission() {
		if (!protocolSubmissions.isEmpty()) {
			...
			if (protocolSubmission == null
					...
					|| protocolSubmissions.get(protocolSubmissions.size() - 1)
							.getSubmissionNumber() > protocolSubmission
							.getSubmissionNumber()) {
				protocolSubmission = protocolSubmissions
						.get(protocolSubmissions.size() - 1);
			}
		} else {
			protocolSubmission = getProtocolSubmissionNewInstanceHook();
			// TODO : the update protocol rule may not like null
			protocolSubmission
					.setProtocolSubmissionType(getProtocolSubmissionTypeNewInstanceHook());
			protocolSubmission
					.setSubmissionStatus(getProtocolSubmissionStatusNewInstanceHook());
		}
		// }
	...
	private void refreshReferenceObject(ProtocolSubmissionBase submission) {
		...
		if (StringUtils.isNotBlank(submission.getSubmissionTypeQualifierCode())
			...
			submission
					.refreshReferenceObject("protocolSubmissionQualifierType");
		}
	...
	public ProtocolActionBase getLastProtocolAction() {
		...
		Collections.sort(protocolActions, new Comparator<ProtocolActionBase>() {
			public int compare(ProtocolActionBase action1,
				...
				return action2.getActualActionDate().compareTo(
						action1.getActualActionDate());
			}
		});
	...
	public void setProtocolSubmissions(
			List<ProtocolSubmissionBase> protocolSubmissions) {
	...
	public void setAttachmentProtocols(
	...
	public void setProtocolAmendRenewals(
			List<ProtocolAmendRenewalBase> protocolAmendRenewals) {
	...
	// this method was modified during IRB backfit merge with the assumption
	// that these changes are applicable to both IRB and IACUC
	public void merge(ProtocolBase amendment, boolean mergeActions) {
		// set this value here, since it is applicable regardless of which
		// modules are amended
		this.lastApprovalDate = amendment.getLastApprovalDate();
		List<ProtocolAmendRenewModuleBase> modules = amendment
				.getProtocolAmendRenewal().getModules();
		for (ProtocolAmendRenewModuleBase module : modules) {
			merge(amendment, module.getProtocolModuleTypeCode());
		}
		if (amendment.isRenewalWithoutAmendment()
				&& isRenewalWithNewAttachment(amendment)) {
		...
		// if renewal, then set the expiration date to be the new one provided
		// during renewal approval
	...
	// this method can be overridden to customize the conditions when the
	// expiration date is to be updated for the main protocol
	...
	private boolean isRenewalWithNewAttachment(ProtocolBase renewal) {
		...
		for (ProtocolAttachmentProtocolBase attachment : renewal
				.getAttachmentProtocols()) {
	...
	public abstract void merge(ProtocolBase amendment,
			String protocolModuleTypeCode);

	protected void mergeProtocolQuestionnaire(ProtocolBase amendment) {
		// TODO : what if user did not edit questionnaire at all, then
		// questionnaire will be wiped out ?
	...
	protected void removeOldQuestionnaire() {
		...
		if (!answerHeaders.isEmpty()
				&& answerHeaders.get(0).getAnswerHeaderId() != null) {
	...
	protected void amendQuestionnaire(ProtocolBase amendment) {
		...
		if (!answerHeaders.isEmpty()) {
			for (AnswerHeader answerHeader : answerHeaders) {
				...
				answerHeader.setModuleSubItemKey(this.getSequenceNumber()
						.toString());
			}
	 ...
	 */
	public abstract List<AnswerHeader> getAnswerHeaderForProtocol(
	...
	protected void mergeProtocolSubmission(ProtocolBase amendment) {
		List<ProtocolSubmissionBase> submissions = (List<ProtocolSubmissionBase>) deepCopy(amendment
	...
	protected void mergeResearchAreas(ProtocolBase amendment) {
		setProtocolResearchAreas((List<ProtocolResearchAreaBase>) deepCopy(amendment
				.getProtocolResearchAreas()));
	}

	@SuppressWarnings("unchecked")
	protected void mergeFundingSources(ProtocolBase amendment) {
		setProtocolFundingSources((List<ProtocolFundingSourceBase>) deepCopy(amendment
				.getProtocolFundingSources()));
	}

	@SuppressWarnings("unchecked")
	protected void mergeReferences(ProtocolBase amendment) {
		setProtocolReferences((List<ProtocolReferenceBase>) deepCopy(amendment
	...
	protected void mergeOrganizations(ProtocolBase amendment) {
		setProtocolLocations((List<ProtocolLocationBase>) deepCopy(amendment
	...
	protected void mergeAttachments(ProtocolBase amendment) {
		...
		for (ProtocolAttachmentProtocolBase attachment : (List<ProtocolAttachmentProtocolBase>) deepCopy(amendment
				.getAttachmentProtocols())) {
			...
			if (attachment.isDeleted()
					&& KraServiceLocator
							.getService(ProtocolAttachmentService.class)
							.isNewAttachmentVersion(
									(ProtocolAttachmentProtocolBase) attachment)) {
				attachmentProtocols.add(attachment);
				attachment.setProtocol(this);
			}
		}
	...
	private void removeDeletedAttachment() {
		...
		for (ProtocolAttachmentProtocolBase attachment : this
				.getAttachmentProtocols()) {
			attachment.setProtocol(this);
			if (attachment.isDeleted()) {
	...
	/*
	 * This is to restore attachments from protocol to amendment when
	 * 'attachment' section is unselected. The attachment in amendment may have
	 * been modified. delete 'attachment' need to be careful. - if the 'file' is
	 * also used in other 'finalized' attachment, then should remove this file
	 * reference from attachment otherwise, the delete will also delete any
	 * attachment that reference to this file
	 */
	protected void restoreAttachments(ProtocolBase protocol) {
		...
		for (ProtocolAttachmentProtocolBase attachment : this
			...
			if (attachment.isFinal()) {
				...
				// in amendment, DRAFT & DELETED must be new attachment because
				// DELETED
	...
	private boolean fileIsReferencedByOther(Long fileId) {
		...
		return getBusinessObjectService().countMatching(
				getProtocolAttachmentProtocolClassHook(), fieldValues) > 1;
	...
	protected void mergeNotepads(ProtocolBase amendment) {
		...
		if (amendment.getNotepads() != null) {
			for (ProtocolNotepadBase notepad : (List<ProtocolNotepadBase>) deepCopy(amendment
					.getNotepads())) {
	...
	protected void mergeSpecialReview(ProtocolBase amendment) {
		setSpecialReviews((List<ProtocolSpecialReviewBase>) deepCopy(amendment
				.getSpecialReviews()));
	...
	protected void mergePersonnel(ProtocolBase amendment) {
		setProtocolPersons((List<ProtocolPersonBase>) deepCopy(amendment
				.getProtocolPersons()));
		for (ProtocolPersonBase person : protocolPersons) {
			Integer nextPersonId = getSequenceAccessorService()
					.getNextAvailableSequenceNumber("SEQ_PROTOCOL_ID")
					.intValue();
			person.setProtocolPersonId(nextPersonId);
			for (ProtocolAttachmentPersonnelBase protocolAttachmentPersonnel : person
					.getAttachmentPersonnels()) {
				protocolAttachmentPersonnel.setId(null);
				protocolAttachmentPersonnel.setPersonId(person
						.getProtocolPersonId());
				protocolAttachmentPersonnel.setProtocolId(getProtocolId());
				protocolAttachmentPersonnel
						.setProtocolNumber(getProtocolNumber());
			}
		}
	}
	...
	protected void mergeOthers(ProtocolBase amendment) {
		...
		if (amendment.getProtocolDocument().getCustomAttributeDocuments() == null
				|| amendment.getProtocolDocument()
						.getCustomAttributeDocuments().isEmpty()) {
			amendment.getProtocolDocument().initialize();
		}
		for (Entry<String, CustomAttributeDocument> entry : protocolDocument
				.getCustomAttributeDocuments().entrySet()) {
			CustomAttributeDocument cad = amendment.getProtocolDocument()
					.getCustomAttributeDocuments().get(entry.getKey());
			entry.getValue().getCustomAttribute()
					.setValue(cad.getCustomAttribute().getValue());
		}
	}
	...
	protected void addAdditionalInfoSummary(ProtocolSummary protocolSummary) {
		...
		additionalInfoSummary.setFdaApplicationNumber(this
				.getFdaApplicationNumber());
	...
	protected void addSpecialReviewSummaries(ProtocolSummary protocolSummary) {
		for (ProtocolSpecialReviewBase specialReview : getSpecialReviews()) {
			...
			specialReviewSummary.setType(specialReview.getSpecialReviewType()
					.getDescription());
			if (specialReview.getApprovalType() == null) {
				specialReview.refreshReferenceObject("approvalType");
			}
			specialReviewSummary.setApprovalStatus(specialReview
					.getApprovalType().getDescription());
			specialReviewSummary.setProtocolNumber(specialReview
					.getProtocolNumber());
			specialReviewSummary.setApplicationDate(specialReview
					.getApplicationDate());
			specialReviewSummary.setApprovalDate(specialReview
					.getApprovalDate());
			specialReviewSummary.setExpirationDate(specialReview
					.getExpirationDate());
			if (specialReview.getSpecialReviewExemptions() == null) {
				specialReview.refreshReferenceObject("specialReviewExemptions");
			}
			specialReviewSummary.setExemptionNumbers(specialReview
					.getSpecialReviewExemptions());
	...
	protected void addOrganizationSummaries(ProtocolSummary protocolSummary) {
		for (ProtocolLocationBase organization : this.getProtocolLocations()) {
			...
			organizationSummary.setOrganizationId(organization
					.getOrganizationId());
			organizationSummary.setName(organization.getOrganization()
					.getOrganizationName());
			organizationSummary.setType(organization
					.getProtocolOrganizationType().getDescription());
			organizationSummary.setContactId(organization.getRolodexId());
			organizationSummary.setContact(organization.getRolodex());
			organizationSummary.setFwaNumber(organization.getOrganization()
	...
	protected void addFundingSourceSummaries(ProtocolSummary protocolSummary) {
		for (ProtocolFundingSourceBase source : getProtocolFundingSources()) {
			...
			fundingSourceSummary.setFundingSourceType(source
					.getFundingSourceType().getDescription());
			fundingSourceSummary.setFundingSource(source
					.getFundingSourceNumber());
			fundingSourceSummary.setFundingSourceNumber(source
					.getFundingSourceNumber());
			fundingSourceSummary.setFundingSourceName(source
					.getFundingSourceName());
			fundingSourceSummary.setFundingSourceTitle(source
					.getFundingSourceTitle());
	...
	protected void addAttachmentSummaries(ProtocolSummary protocolSummary) {
		for (ProtocolAttachmentProtocolBase attachment : getActiveAttachmentProtocols()) {
			if (!attachment.isDeleted()) {
				...
				attachmentSummary.setAttachmentType("Protocol: "
						+ attachment.getType().getDescription());
				attachmentSummary.setDescription(attachment.getDescription());
				attachmentSummary
						.setDataLength(attachment.getFile().getData() == null ? 0
								: attachment.getFile().getData().length);
				attachmentSummary.setUpdateTimestamp(attachment
						.getUpdateTimestamp());
		...
		for (ProtocolPersonBase person : getProtocolPersons()) {
			for (ProtocolAttachmentPersonnelBase attachment : person
					.getAttachmentPersonnels()) {
				AttachmentSummary attachmentSummary = new AttachmentSummary();
				attachmentSummary.setAttachmentId(attachment.getId());
				attachmentSummary.setFileType(attachment.getFile().getType());
				attachmentSummary.setFileName(attachment.getFile().getName());
				attachmentSummary.setAttachmentType(person.getPersonName()
						+ ": " + attachment.getType().getDescription());
				attachmentSummary.setDescription(attachment.getDescription());
				attachmentSummary
						.setDataLength(attachment.getFile().getData() == null ? 0
	...
	protected void addResearchAreaSummaries(ProtocolSummary protocolSummary) {
		for (ProtocolResearchAreaBase researchArea : getProtocolResearchAreas()) {
			...
			researchAreaSummary.setResearchAreaCode(researchArea
					.getResearchAreaCode());
			researchAreaSummary.setDescription(researchArea.getResearchAreas()
					.getDescription());
	...
	protected void addPersonnelSummaries(ProtocolSummary protocolSummary) {
		for (ProtocolPersonBase person : getProtocolPersons()) {
			...
			personnelSummary.setRoleName(person.getProtocolPersonRole()
					.getDescription());
			if (person.getAffiliationTypeCode() == null) {
				...
				personnelSummary.setAffiliation(person.getAffiliationType()
						.getDescription());
			}
			for (ProtocolUnitBase unit : person.getProtocolUnits()) {
				personnelSummary.addUnit(unit.getUnitNumber(),
						unit.getUnitName());
			}
	...
	public void resetPersistenceStateForNotifications() {
		for (ProtocolActionBase action : protocolActions) {
			for (KcNotification notification : action
					.getProtocolNotifications()) {
	...
	public void populateAdditionalQualifiedRoleAttributes(
	...
	public boolean isAmendment() {
		return protocolNumber != null
				&& protocolNumber.contains(AMENDMENT_LETTER);
	}

	public boolean isRenewal() {
		return protocolNumber != null
				&& protocolNumber.contains(RENEWAL_LETTER);
	}

	public boolean isRenewalWithAmendment() {
		return isRenewal()
				&& CollectionUtils.isNotEmpty(this.getProtocolAmendRenewal()
						.getModules());
	}

	public boolean isRenewalWithoutAmendment() {
		return isRenewal()
				&& CollectionUtils.isEmpty(this.getProtocolAmendRenewal()
						.getModules());
	}
	...
	public String getAmendedProtocolNumber() {
		if (isAmendment()) {
			return StringUtils.substringBefore(getProtocolNumber(),
					AMENDMENT_LETTER.toString());

		} else if (isRenewal()) {
			return StringUtils.substringBefore(getProtocolNumber(),
					RENEWAL_LETTER.toString());
		...
		for (ProtocolAttachmentProtocolBase attachment1 : getAttachmentProtocols()) {
			if (attachment1.isDraft()) {
				...
				for (ProtocolAttachmentProtocolBase attachment2 : getAttachmentProtocols()) {
					if (attachment1.getDocumentId().equals(
							attachment2.getDocumentId())
							&& attachment1.getAttachmentVersion() < attachment2
	 ...
	 * @return
	 */
	// TODO the method code below could be restructured to combine the two for
	// loops into one loop.
	public List<ProtocolAttachmentProtocolBase> getActiveAttachmentProtocolsNoDelete() {
		...
		for (ProtocolAttachmentProtocolBase attachment : getActiveAttachmentProtocols()) {
			if (documentIds.isEmpty()
					|| !documentIds.contains(attachment.getDocumentId())) {
	...
	protected SequenceAccessorService getSequenceAccessorService() {
		if (sequenceAccessorService == null) {
			sequenceAccessorService = (SequenceAccessorService) KraServiceLocator
	...
	public void setLookupPendingPIActionProtocol(
	...
	public void setLookupActionAmendRenewProtocol(
			boolean lookupActionAmendRenewProtocol) {
	...
	public void setLookupActionRequestProtocol(
			boolean lookupActionRequestProtocol) {
	...
	public boolean isFollowupAction(String actionTypeCode) {
		return (getLastProtocolAction() == null || StringUtils
				.isBlank(getLastProtocolAction().getFollowupActionCode())) ? false
				: actionTypeCode.equals(getLastProtocolAction()
	...
	public void setProtocolAttachmentFilter(
			ProtocolAttachmentFilterBase protocolAttachmentFilter) {
	...
	public List<ProtocolAttachmentProtocolBase> getFilteredAttachmentProtocols() {
		...
		if (attachmentFilter != null
				&& StringUtils.isNotBlank(attachmentFilter.getFilterBy())) {
			for (ProtocolAttachmentProtocolBase attachment1 : getAttachmentProtocols()) {
				if ((attachment1.getTypeCode()).equals(attachmentFilter
						.getFilterBy())) {
		...
		if (attachmentFilter != null
				&& StringUtils.isNotBlank(attachmentFilter.getSortBy())) {
			Collections.sort(filteredAttachments,
					attachmentFilter.getProtocolAttachmentComparator());
		}
	...
	public void cleanupSpecialReviews(ProtocolBase srcProtocol) {
		List<ProtocolSpecialReviewBase> srcSpecialReviews = srcProtocol
				.getSpecialReviews();
		List<ProtocolSpecialReviewBase> dstSpecialReviews = getSpecialReviews();
		for (int i = 0; i < srcSpecialReviews.size(); i++) {
			ProtocolSpecialReviewBase srcSpecialReview = srcSpecialReviews
					.get(i);
			ProtocolSpecialReviewBase dstSpecialReview = dstSpecialReviews
					.get(i);
			// copy exemption codes, since they are transient and ignored by
			// deepCopy()
	...
	public boolean isMemberInProtocolPersonnel(CommitteeMembershipBase member) {
		...
		for (ProtocolPersonBase protocolPerson : this.protocolPersons) {
			if (StringUtils.isNotBlank(member.getPersonId())
				...
				if ((null != member.getRolodexId())
						&& (null != protocolPerson.getRolodexId())) {
					if (member.getRolodexId().equals(
	 ...
	 */
	public List<CommitteeMembershipBase> filterOutProtocolPersonnel(
	...
	public List<ProtocolActionBase> getSortedActions() {
		if (sortedActions == null) {
			...
			for (ProtocolActionBase action : getProtocolActions()) {
				sortedActions.add((ProtocolActionBase) ObjectUtils
						.deepCopy(action));
			}

			Collections.sort(sortedActions,
					new Comparator<ProtocolActionBase>() {
						public int compare(ProtocolActionBase action1,
								ProtocolActionBase action2) {
							return action1.getActionId().compareTo(
									action2.getActionId());
						}
					});
	...
	public void reconcileActionsWithSubmissions() {
		...
		for (ProtocolSubmissionBase submission : getProtocolSubmissions()) {
			submissionNumberMap.put(submission.getSubmissionNumber(),
					submission);
		}

		for (ProtocolActionBase action : getProtocolActions()) {
			if (action.getSubmissionNumber() != null
					&& !action.getSubmissionNumber().equals(0)) {
				if (submissionNumberMap.get(action.getSubmissionNumber()) != null) {
					action.setSubmissionIdFk(submissionNumberMap.get(
							action.getSubmissionNumber()).getSubmissionId());
				}
			}
		}
	}
