 ...
 */
public abstract class ProtocolAttachmentProtocolBase extends
	...
	// 1-Complete, 2-Incomplete. an attachment status must be 'complete' before
	// this protocol can be submitted.
	...
	// All new files are 'Draft'. When protocol is versioned, all 'Draft' become
	// 'Finalized'
	...
	// an indicator to decide whether to display this file in protocol
	// attachment panel or not
	protected boolean active = true;

	// an indicator of whether this file has been changed/replaced or not. This
	// is if documentstatus is 1 or 3.
	// if it is changed, then the updateuser and updatetimestamp of this record
	// will be updated.
	protected boolean changed = false;

	// protocol number corresponding to the newest timestamp when this
	// attachment was modified/attached
	private String sourceProtocolNumber;

	// the amendment/renewal number corresponding to the newest timestamp when
	// this attachment was modified/attached
	 ...
	 * 
	 * @param status
	 *            the ProtocolBase Attachment ProtocolBase Status
	 */
	 ...
	 * 
	 * @param contactName
	 *            the ProtocolBase Attachment ProtocolBase Contact Name
	 */
	 ...
	 *            the ProtocolBase Attachment ProtocolBase Contact Email Address
	 */
	 ...
	 *            the ProtocolBase Attachment ProtocolBase Contact Phone Number
	 */
	 ...
	 * 
	 * @param comments
	 *            the ProtocolBase Attachment ProtocolBase comments
	 */
	...
	public List<ProtocolAttachmentProtocolBase> getVersions() {
		...
		// this.versions.addAll(KraServiceLocator.getService(ProtocolAttachmentService.class).getAttachmentsWithOlderFileVersions(this,
		// ProtocolAttachmentProtocolBase.class));
		// need this refresh here. change and save will not update this list
		...
		for (ProtocolAttachmentProtocolBase attachment : this.getProtocol()
		...
		Collections.sort(this.versions,
				new Comparator<ProtocolAttachmentProtocolBase>() {

					public int compare(
							ProtocolAttachmentProtocolBase attachment1,
							ProtocolAttachmentProtocolBase attachment2) {
						return attachment2.getUpdateTimestamp().compareTo(
								attachment1.getUpdateTimestamp());
					}
				});
	...
	public int hashCode() {
		...
		result = prime
				* result
				+ ((contactEmailAddress == null) ? 0 : contactEmailAddress
						.hashCode());
		result = prime * result
		...
		// result = prime * result + ((documentId == null) ? 0 :
		// documentId.hashCode());
		result = prime * result
	...
	public static enum PropertyName {
		...
		COMMENTS("comments"), EMAIL("contactEmailAddress"), CONTACT_NAME(
				"contactName"), PHONE("contactPhoneNumber"), STATUS_CODE(
				"statusCode"), DOCUMENT_STATUS_CODE("documentStatusCode"), ATTACHMENT_VERSION(
				"attachmentVersion"), CREATE_TIMESTAMP("createTimestamp");
	...
	public void setUpdateTimestamp(Timestamp updateTimestamp) {
		if (getDocumentStatusCode() == null || updateTimestamp == null
				|| getUpdateTimestamp() == null || isAttachmentUpdated()) {
	...
	public void setUpdateUser(String updateUser) {
		if (getDocumentStatusCode() == null || updateUser == null
				|| getUpdateUser() == null || isAttachmentUpdated()) {
	...
	/**
	 * Returns the source protocol number for when this attachment was last
	 * added in.
	 */
	public String getSourceProtocolNumber() {
		if (sourceProtocolNumber == null) {
			if (getFileId() != null) {
				// find the protocol number for this attachment's most recent
				...
				List<ProtocolAttachmentProtocol> protocolAttachmentProtocols = (List<ProtocolAttachmentProtocol>) businessObjectService
						.findMatchingOrderBy(ProtocolAttachmentProtocol.class,
								param, "id", true);
				ProtocolAttachmentProtocol protocolAttachmentProtocol = protocolAttachmentProtocols
						.get(0);
				sourceProtocolNumber = protocolAttachmentProtocol
						.getProtocolNumber();
			}
			// Null FileId means attachment newly added and is not in the
			// database yet
			else {
	...
	public String getSourceProtocolAmendRenewalNumber() {
		if (sourceProtocolAmendRenewalNumber == null) {
			if (getSourceProtocolNumber().length() >= 10) {
				sourceProtocolAmendRenewalNumber = sourceProtocolNumber
						.substring(10);
			} else {
	 ...
	 * protocol again next time getSourceProtocolNumber is called. You may want
	 * to do this if the attachment is saved or modified in some way which would
	 * cause the source protocol to change.
	 */
	...
	protected void prePersist() {
		...
		if (getCreateTimestamp() == null) {
			setCreateTimestamp(((DateTimeService) KraServiceLocator
					.getService(Constants.DATE_TIME_SERVICE_NAME))
					.getCurrentTimestamp());
		}
	}
}
