...
public abstract class ResearchDocumentBase extends TransactionalDocumentBase {
	...
	public void prepareForSave() {
		...
		// Since the UPDATE_USER column is only VACHAR(60), we need to truncate this string if it's longer than 60
		// characters
	...
	public DocumentNextvalue getDocumentNextvalueBo( String propertyName ) {
		...
		// following should not happen because it already got the next value for this property before calling this for
		// updating
		DocumentNextvalue documentNextvalue = new DocumentNextvalue();
	 ...
	 *            The customAttributeDocuments to set.
	 */
	...
	/*
	 * Please keep in mind that the system checks for existing locks by matching the lock descriptor string against all
	 * the descriptors for the given document while creating new locks. So elements that change over the document's
	 * functional process, like updateTimeStamp should never be added to the lock descriptor since this will cause new
	 * locks to be created upon every save.
	 */
	@Override
	...
	/*
	 * Gets the unique identifier of the BO associated with a document. Gets the proposalNumber, awardNumber,
	 * negotiationNumber etc.
	 */
	public abstract String getDocumentBoNumber();

	/**
	 * Get the list of roles for the document along with each of the individuals in those roles. This information will
	 * be serialized into XML for workflow routing purposes. It is expected that this method will be overridden by
	 * derived classes.
	 * 
	...
	/**
	 * In documents that support it, this method should answer T/F for a SplitNode question regarding routing. The
	 * SimpleBooleanSplitNode will supply the route node name which implementations should use to answer the question.
	 * 
	 * For example, isHierarchyChild would be answered by ProposalDevelopementDocument as T if the document is a child
	 * within a proposal hierarchy and false otherwise.
	 * 
	 ...
	 * 
	 * @param routeNodeName
	 *            The name of the routeNode as it appears in the document workflow definition.
	 * @return boolean value representing the answer.
	 * @throws UnsupportedOperationException
	 *             if the document does not support answering a split node question, or if it does not support answering
	 *             the question for the supplied route name.
	 * 
	 */
