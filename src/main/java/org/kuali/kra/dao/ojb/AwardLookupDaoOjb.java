...
public class AwardLookupDaoOjb extends LookupDaoOjb implements AwardLookupDao {
	...
	private boolean checkAwardHasActiveTnMDocument( Award awardSearckBo ) {
		...
		List<TimeAndMoneyDocument> timeAndMoneyDocuments = (List<TimeAndMoneyDocument>) getPersistenceBrokerTemplate().getCollectionByQuery( query );
		...
		if ( !timeAndMoneyDocuments.isEmpty() ) {
			...
			WorkflowDocument tnmWorkFlowDoc = documentHeader == null || !documentHeader.hasWorkflowDocument() ? null : documentHeader.getWorkflowDocument();
