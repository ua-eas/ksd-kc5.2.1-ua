...
public class ProposalDevelopmentActionsAction extends ProposalDevelopmentAction implements AuditModeAction {
	...
	/**
	 * Updates an Editable Proposal Field and adds it to the Proposal Change History
	 * 
	 * @param mapping
	 *            the Struct's Action Mapping.
	 * @param form
	...
	public ActionForward reject( ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response )
		...
		// ProposalDevelopmentRejectionBean bean =
		// ((ProposalDevelopmentForm)form).getProposalDevelopmentRejectionBean();

		// ProposalHierarchyService phService = KraServiceLocator.getService(ProposalHierarchyService.class);
		// phService.rejectProposalDevelopmentDocument(pDoc.getDevelopmentProposal().getProposalNumber(),
		// bean.getRejectReason(), GlobalVariables
	...
	public ActionForward route( ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response ) throws Exception {
		...
		proposalDevelopmentForm.setAuditActivated( true );
		// success=KraServiceLocator.getService(KualiRuleService.class).applyRules(new
		// DocumentAuditEvent(proposalDevelopmentForm.getProposalDevelopmentDocument()));
	...
	/*
	 * Used by isValidSubmission and reused in submitToSponsor, this method expects existence of at least a WARNING
	 * state/status. i.e. at least one AuditError exists in Constants.AUDIT_WARNINGS audit cluster category. It also
	 * probes the cluster for clusters belonging to AUDIT_ERRORS. If found, the status/state immediately escalates to,
	 * and returns ERROR.
	 */
	...
	private int isValidSubmission( ProposalDevelopmentForm proposalDevelopmentForm ) {
		...
		/*
		 * If this proposal is a continuation from another proposal, it is illegal for it to have a New Proposal Type
		 * and Application Submission Type.
		 */
		...
		if ( ( proposalDevelopmentDocument.getDevelopmentProposal().getContinuedFrom() != null ) && isNewProposalType( proposalDevelopmentDocument ) && isSubmissionApplication( proposalDevelopmentDocument ) ) {
			state = ERROR;
			// GlobalVariables.getMessageMap().putError("someKey",
			// KeyConstants.ERROR_RESUBMISSION_INVALID_PROPOSALTYPE_SUBMISSIONTYPE);
		}
			...
			/*
			 * Don't know what to do about the Audit Rules. The parent class invokes the Audit rules from its execute()
			 * method. It will stay this way for the time being because I this is what the code did before it was
			 * refactored.
			 */
			boolean auditPassed = new AuditActionHelper().auditUnconditionally( proposalDevelopmentDocument );

			/*
			 * Check for S2S validation if we have a Grants.gov Opportunity. If there is no Grants.gov Opportunity, then
			 * the user wants to submit the proposal manually (printing and sending via snail mail).
			 */
			...
			/*
			 * If either reported an error, then we have to check to see if we only have warnings or if we also have
			 * some errors.
			 */
	...
	/**
	 * Submit the proposal to the sponsor without any further error checking. Can be invoked if the user wishes to
	 * ignore an warnings that occurred when first trying to submit the proposal.
	 * 
	 * @param mapping
	 * @param form
	 * @param request
	 ...
	 * 
	 * @param instProposalNumber
	 * @param instProposalSequenceNumber
	 *            TODO
	 * @param proposalDevelopmentDocument
	 */
	...
	public ActionForward printSponsorForms( ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response ) throws Exception {
		...
		// TODO fix - printing code does not catch null fields in proposal and does not fail gracefully when certain
		// fields are blank
		try {
		...
		// String ReportName = proposalNumber.concat("_" +
		// proposalDevelopmentDocument.getDevelopmentProposal().getSponsorCode()).concat(Constants.PDF_FILE_EXTENSION);
	...
	/**
	 * 
	 * This method is for audit rule to forward to the page that the audit error fix is clicked. This is an example for
	 * budget audit error forward from proposal to budget page.
	 * 
	 * @param mapping
	 * @param form
	 * @param request
	...
	/**
	 * 
	 * This method is for audit rule to forward to the Budget Parameters page when the associated audit error fix is
	 * clicked.
	 * 
	 * @param mapping
	 * @param form
	...
	public ActionForward disapprove( ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response )
		...
		if ( StringUtils.isNotBlank( request.getParameter( KRADConstants.QUESTION_REASON_ATTRIBUTE_NAME ) ) ) {
			...
			if ( instPropId != null ) {
				// we must have already submitted this proposal to sponsor so we need to create a new version of the
				// inst prop
	...
	/**
	 * Prepare current report (i.e. Awards that selected person is on) {@inheritDoc}
	 */
