...
public class ProposalDevelopmentAction extends BudgetParentActionBase {
	...
	public ActionForward execute( ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response ) throws Exception {
		 ...
		 * proposalDevelopmentForm.setAdditionalDocInfo1(new
		 * ConcreteKeyValue("datadictionary.Sponsor.attributes.sponsorCode.label"
		 * ,proposalDevelopmentForm.getProposalDevelopmentDocument().getSponsorCode())); }
		 * if(proposalDevelopmentForm.getProposalDevelopmentDocument().getPrincipalInvestigator()!=null){
		 * proposalDevelopmentForm.setAdditionalDocInfo2(new
		 * ConcreteKeyValue("${Document.DataDictionary.ProposalDevelopmentDocument.attributes.sponsorCode.label}"
		 * ,proposalDevelopmentForm.getProposalDevelopmentDocument().getPrincipalInvestigator().getFullName())); }
		 */
	...
	/**
	 * Do nothing. Used when the Proposal is in view-only mode. Instead of saving the proposal when the tab changes, we
	 * simply do nothing.
	 * 
	 * @param mapping
	 * @param form
	...
	/**
	 * 
	 * Updates portions of the proposal that are not persisted and are based on whether the sponsor is NIH or not
	 * 
	 * @param document
	 */
	...
	public ActionForward save( ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response ) throws Exception {
		...
		// if the proposal hasn't been saved yet, the s2sopp proposal number will be null. We need to save it in the
		// form until we
		...
		// If validation is turned on, take the user to the proposal actions page (which contains the validation panel,
		// which auto-expands)
		if ( proposalDevelopmentForm.isAuditActivated() ) {
	 ...
	 * outside of the one currently being saved, must be updated from the database to make sure the sessions do not
	 * stomp changes already persisted by another session.
	 * 
	 * @param pdForm
	 * @throws Exception
	 */
	protected void updateProposalDocument( ProposalDevelopmentForm pdForm ) throws Exception {
		...
		if ( updatedDocCopy != null ) {
			...
			// For Budget and Narrative Lock regions, this is the only way in which a Proposal Document might get
			// updated
			...
			// if no other session has saved this document we should be updating with same version number, but no easy
			// way to know if it has been
	...
	/*
	 * The updatePD has some issue, such as if person is deleted, and the person attachment is also deleted. However,
	 * the updateProposalDocument recover everything from DB. so, add this method to delete the deleted, but not saved
	...
	public ActionForward abstractsAttachments( ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response ) {
		...
		/*
		 * Save the current set of narratives. In some cases, a user can view the narrative panel info, but is not
		 * allowed to change it. We will make a copy of the original narratives to use for comparison when a save
		 * occurs. If a user attempted to change a narrative they were not authorized to, then an error will be posted.
		 */
	...
	/**
	 * Create the original set of Proposal Users for a new Proposal Development Document. The creator the proposal is
	 * assigned to the AGGREGATOR role.
	 */
	...
	protected void loadDocumentInForm( HttpServletRequest request, ProposalDevelopmentForm proposalDevelopmentForm )
		...
		// Set lead unit on form when copying a document. This is needed so the lead unit shows up on the
		// "Copy to New Document" panel under Proposal Actions.
	...
	public ActionForward printForms( ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response ) throws Exception {
		...
		if ( attachmentDataSource == null || attachmentDataSource.getContent() == null || attachmentDataSource.getContent().length == 0 ) {
			// KRACOEUS-3300 - there should be GrantsGov audit errors in this case, grab them and display them as normal
			// errors on
	 ...
	 * ProposalColumnsToAlter lookup buttons that can be rendered on the Proposal Data Override tab. The execute method
	 * in this class puts this list into the form. The Proposal Data Override tag file then calls
	...
	/**
	 * 
	 * @see org.kuali.rice.kns.web.struts.action.KualiDocumentActionBase#recall(org.apache.struts.action.ActionMapping,
	 *      org.apache.struts.action.ActionForm, javax.servlet.http.HttpServletRequest,
	 *      javax.servlet.http.HttpServletResponse)
	 */
	@SuppressWarnings( "deprecation" )
	...
	public ActionForward recall( ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response ) throws Exception {
		...
		if ( resp.forward != null ) {
			// forward either to a fresh display of the question, or to one with "blank reason" error message due to the
			// previous answer,
