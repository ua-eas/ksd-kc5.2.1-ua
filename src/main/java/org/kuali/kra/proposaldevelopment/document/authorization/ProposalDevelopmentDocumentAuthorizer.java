...
/**
 * The Proposal Development Document Authorizer. Primarily responsible for determining if a user has permission to
 * create/modify/view proposals.
 *
...
public class ProposalDevelopmentDocumentAuthorizer extends KcTransactionalDocumentAuthorizerBase {
	...
	private void setNarrativePermissions( String userId, ProposalDevelopmentDocument doc, Set<String> editModes ) {
		...
		for ( ProposalPersonBiography ppb : doc.getDevelopmentProposal().getPropPersonBios() ) {
			...
			// boolean canReplace taskAuthorizationService.isAuthorized(userId, new
			// NarrativeTask(TaskName.REPLACE_NARRATIVE, doc, ppb));
	...
	/**
	 * Does the user have permission to create a proposal. Use the Unit Authorization Service to determine if the user
	 * has the CREATE_PROPOSAL permission in any unit.
	 * 
	 * @param user
	 *            the user
	...
	public boolean canBlanketApprove( Document document, Person user ) {
		...
		canBlanketApprove &= canExecuteProposalTask( user.getPrincipalName(), (ProposalDevelopmentDocument) document, TaskName.PROPOSAL_HIERARCHY_CHILD_WORKFLOW_ACTION );
