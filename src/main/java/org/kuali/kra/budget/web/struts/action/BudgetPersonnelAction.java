...
import edu.arizona.kra.bo.AcademicAndSummerStarts;
import edu.arizona.kra.budget.web.struts.action.SpecialDateRange;
...
public class BudgetPersonnelAction extends BudgetExpensesAction {
	...
	private static final String ACADEMIC_APPOINTMENT_TYPE_CODE = "3";
    private static final String SUMMER_APPOINTMENT_TYPE_CODE = "2";
    private static final int SUMMER_PERIOD_END_DAY = 14;
    private static final int SUMMER_PERIOD_END_MONTH = 8;
    private static final int ACADEMIC_PERIOD_END_DAY = 14;
    private static final int ACADEMIC_PERIOD_END_MONTH = 5;
	...
	private boolean areCalendarDatesEqual(Calendar firstDate, Calendar secondDate){
	...
	/**
	 * Breaks down a Budget Period into Academic periods or Summer Periods depending upon the Appointment parameter and
	 * returns an <code>ArrayList</code> of such periods in the form of <code>SpecialDateRange</code> objects
	 * 
	 * @param budgetPeriodStart
	 * @param budgetPeriodEnd
	 * @param personAppointment
	 * @return
	 */
	private ArrayList<SpecialDateRange> getAcademicOrSummerPeriods(Date budgetPeriodStart, Date budgetPeriodEnd, 
			SpecialDateRange.Appointment personAppointment) {
		// TODO Auto-generated method stub
		Calendar calBudgetPeriodStartDate = new GregorianCalendar();
		Calendar calBudgetPeriodEndDate = new GregorianCalendar();
		
		calBudgetPeriodStartDate.setTime(budgetPeriodStart);
		calBudgetPeriodEndDate.setTime(budgetPeriodEnd);
		
		Calendar calLastAcademicDay = null;
		Calendar calLastSummerDay = null;
		
		BusinessObjectService businessObjectService = KraServiceLocator.getService(BusinessObjectService.class);
		Map queryMap = new HashMap();
        queryMap.put("schoolYear", calBudgetPeriodStartDate.get(Calendar.YEAR) );
        List<AcademicAndSummerStarts> acadAndSummerStarts = (List<AcademicAndSummerStarts>) businessObjectService.findMatching(AcademicAndSummerStarts.class, queryMap);
        Date summerPeriodStart = null;
        Date academicPeriodStart = null;
        if(acadAndSummerStarts.size() > 0){
        	summerPeriodStart = acadAndSummerStarts.get(0).getSummerPeriodStart();
        	academicPeriodStart = acadAndSummerStarts.get(0).getAcademicPeriodStart();
        }
        if(summerPeriodStart != null){
        	calLastAcademicDay = new GregorianCalendar();
    		calLastAcademicDay.setTime(summerPeriodStart);
    		calLastAcademicDay.set(Calendar.DAY_OF_MONTH, calLastAcademicDay.get(Calendar.DAY_OF_MONTH)-1);
        }
        else{
        	calLastAcademicDay = new GregorianCalendar(
        			calBudgetPeriodStartDate.get(Calendar.YEAR), 
    				ACADEMIC_PERIOD_END_MONTH - 1, 
    				ACADEMIC_PERIOD_END_DAY);
        }
        if(academicPeriodStart != null){
        	calLastSummerDay = new GregorianCalendar();
        	calLastSummerDay.setTime(academicPeriodStart);
        	calLastSummerDay.set(Calendar.DAY_OF_MONTH, calLastSummerDay.get(Calendar.DAY_OF_MONTH)-1);
        }
        else {
        	calLastSummerDay = new GregorianCalendar(
        			calBudgetPeriodStartDate.get(Calendar.YEAR), 
    				SUMMER_PERIOD_END_MONTH - 1, 
    				SUMMER_PERIOD_END_DAY);
        	academicPeriodStart = new java.sql.Date(
        			(new GregorianCalendar(calBudgetPeriodStartDate.get(Calendar.YEAR), 
    									SUMMER_PERIOD_END_MONTH - 1, 
    									SUMMER_PERIOD_END_DAY + 1)).getTime().getTime());
        }
		
		ArrayList<SpecialDateRange> acadOrSummerPeriods = new ArrayList<SpecialDateRange>();
		SpecialDateRange dateRange = new SpecialDateRange();
		dateRange.setStart(budgetPeriodStart);
		//Following is important for budget periods that are purely within a single academic or summer period
		if(budgetPeriodStart.after(calLastAcademicDay.getTime()) && budgetPeriodStart.before(academicPeriodStart)){
		...
		while(true) {
			...
			else if(areCalendarDatesEqual(calIndex, calLastSummerDay)) {
				dateRange.setEnd(new Date(calLastSummerDay.getTime().getTime()));
				dateRange.setAppointment(SpecialDateRange.Appointment.SUMMER);
	...
	public ActionForward addPersonnelLineItem( ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response ) throws Exception {
		...
		// if the group name is empty or still the default group name(JS error?) then set the group name to the empty
		// string.
		...
		if ( budgetForm.getViewBudgetPeriod() == null || StringUtils.equalsIgnoreCase( budgetForm.getViewBudgetPeriod().toString(), "0" ) ) {
			...
			if ( GlobalVariables.getMessageMap().hasNoErrors() ) {
						...
						if ( newBudgetLineItem.getCostElement().equalsIgnoreCase( budgetLineItem.getCostElement() ) &&
							...
							if ( budgetPersonDetails.getPersonSequenceNumber().intValue() != -1 ) {
								...
								if ( budgetPersonnelExpenseRule.processCheckPersonAddBusinessRules( budgetLineItem ) ) {
									...
									if(appointmentTypeCode.equalsIgnoreCase(ACADEMIC_APPOINTMENT_TYPE_CODE) || 
                                    		appointmentTypeCode.equalsIgnoreCase(SUMMER_APPOINTMENT_TYPE_CODE)){
                                    	budgetPersonDetails.setStartDate(((SpecialDateRange)acadOrSummerPeriods.get(0)).getStart());
                                    	budgetPersonDetails.setEndDate(((SpecialDateRange)acadOrSummerPeriods.get(0)).getEnd());
                                    	budgetPersonDetails.setPeriodTypeCode(appointmentTypeCode.equalsIgnoreCase(ACADEMIC_APPOINTMENT_TYPE_CODE)?
                                    			"2":"4");
                                    	acadOrSummerPeriods.remove(0);
                                    }

									addBudgetPersonnelDetails( budgetForm, budgetPeriod, budgetLineItem, budgetPersonDetails );

									addAdditionalBudgetPersonnelDetails( appointmentTypeCode, acadOrSummerPeriods, budgetPersonDetails, budgetForm, budgetPeriod, budgetLineItem );
								}
							}
				...
				if ( !existingCeGroupCombo ) {
					...
					// newBudgetLineItem.setProposalNumber(budget.getBudgetPeriod(budgetPeriod.getBudgetPeriod() -
					// 1).getProposalNumber());
					// newBudgetLineItem.setBudgetVersionNumber(budget.getBudgetPeriod(budgetPeriod.getBudgetPeriod() -
					// 1).getBudgetVersionNumber());
	...
	private void addAdditionalBudgetPersonnelDetails( String appointmentTypeCode, ArrayList<SpecialDateRange> acadOrSummerPeriods,
			BudgetPersonnelDetails budgetPersonDetails, BudgetForm budgetForm, BudgetPeriod budgetPeriod, BudgetLineItem newBudgetLineItem ) throws Exception {
		if ( appointmentTypeCode.equalsIgnoreCase( ACADEMIC_APPOINTMENT_TYPE_CODE ) ||
				appointmentTypeCode.equalsIgnoreCase( SUMMER_APPOINTMENT_TYPE_CODE ) ) {
			BudgetPersonnelDetails additionalBudgetPersonDetails = null;
			for ( SpecialDateRange sdr : acadOrSummerPeriods ) {
				additionalBudgetPersonDetails = new BudgetPersonnelDetails();
				try {
					ConvertUtils.register( new SqlDateConverter( null ), java.sql.Date.class );
					ConvertUtils.register( new SqlTimestampConverter( null ), java.sql.Timestamp.class );
					BeanUtils.copyProperties( additionalBudgetPersonDetails, (BudgetLineItemBase) budgetPersonDetails );
				} catch ( Exception e ) {
					// following lines are copied from BudgetPersonnelBudgetServiceImpl.copyLineItemToPersonnelDetails
					// However using BudgetPersonnelDetails object to copy from rather than BudgetLineItem
					additionalBudgetPersonDetails.setBudgetId( budgetPersonDetails.getBudgetId() );
					additionalBudgetPersonDetails.setBudgetPeriod( budgetPersonDetails.getBudgetPeriod() );
					additionalBudgetPersonDetails.setLineItemNumber( budgetPersonDetails.getLineItemNumber() );
					additionalBudgetPersonDetails.setCostElement( budgetPersonDetails.getCostElement() );
					additionalBudgetPersonDetails.setCostElementBO( budgetPersonDetails.getCostElementBO() );
					additionalBudgetPersonDetails.setApplyInRateFlag( budgetPersonDetails.getApplyInRateFlag() );
					additionalBudgetPersonDetails.setOnOffCampusFlag( budgetPersonDetails.getOnOffCampusFlag() );
				}
				additionalBudgetPersonDetails.setStartDate( sdr.getStart() );
				additionalBudgetPersonDetails.setEndDate( sdr.getEnd() );
				budgetPersonDetails.setPeriodTypeCode( appointmentTypeCode.equalsIgnoreCase( ACADEMIC_APPOINTMENT_TYPE_CODE ) ?
						"2" : "4" );
				// In absence of the following line, call to addBudgetPersonnelDetails below will throw error
				budgetForm.getNewBudgetPersonnelDetails().setPersonSequenceNumber( 2 );
				// In absence of the following line, BUDGET_PERSONNEL_DETAILS.BUDGET_PERSONNEL_DETAILS_ID contain value
				// 0 (Zero) which is wrong
				additionalBudgetPersonDetails.setBudgetPersonnelLineItemId( null );
				addBudgetPersonnelDetails( budgetForm, budgetPeriod, newBudgetLineItem, additionalBudgetPersonDetails );
			}
		}
	}
	 ...
	 * 
	 * @param mapping
	 *            The mapping associated with this action.
	 * @param form
	 *            The Proposal Development form.
	 * @param request
	 *            the HTTP request
	 * @param response
	 *            the HTTP response
	 ...
	 * <br/>
	 * The confirmation question is extracted from the resource bundle and the parameter {0} is replaced with the name
	 * of the abstract type that will be deleted.
	 * 
	 * @param mapping
	 *            The mapping associated with this action.
	 * @param form
	 ...
	 *            the HTTP response
	 * @param questionId
	 *            String questionId. This needs to be unique for each type of attachment because there are different
