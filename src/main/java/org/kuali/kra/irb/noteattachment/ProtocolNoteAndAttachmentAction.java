...
public class ProtocolNoteAndAttachmentAction extends ProtocolAction {    
    ...
    public ActionForward deleteAttachmentProtocol(ActionMapping mapping, ActionForm form, HttpServletRequest request,
        ...
        org.kuali.kra.protocol.noteattachment.ProtocolAttachmentBase attachment = helper.retrieveExistingAttachmentByType(
                selection, helper.getProtocolAttachmentProtocolClassHook());
        if (isValidContactData(attachment, ATTACHMNENT_PATH + selection + "]")) {
            return confirmDeleteAttachment(mapping, (ProtocolForm) form, request, response,  helper.getProtocolAttachmentProtocolClassHook());
        } else {
            return mapping.findForward(Constants.MAPPING_BASIC);
        }
    }
    ...
    public ActionForward confirmDeleteAttachmentProtocol(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        NotesAttachmentsHelper helper = ((ProtocolForm) form).getNotesAttachmentsHelper();
        return this.deleteAttachment(mapping, (ProtocolForm) form, request, response, helper.getProtocolAttachmentProtocolClassHook());
    }
