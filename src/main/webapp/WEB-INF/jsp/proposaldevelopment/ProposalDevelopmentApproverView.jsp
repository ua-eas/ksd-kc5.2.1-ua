...
<kul:documentPage showDocumentInfo="true" htmlFormAction="proposalDevelopmentApproverView"
	...
	<div id="workarea">
		...
		<c:if
			test="${KualiForm.proposalDevelopmentParameters['enableSummaryQuestionsPanel'].value == 'Y'}">
			<kul:tab tabTitle="Questions" defaultOpen="false"
				...
				<div class="tab-container" align="center">
					<kra-summary:proposalSummaryQuestions transparentBackground="${isTopPanel }"
						bean="${KualiForm.questionnaireHelper}" property="questionnaireHelper" parentTab="Questions" />
					...
					<kra-summary:proposalSummaryQuestions transparentBackground="${isTopPanel }"
						bean="${KualiForm.s2sQuestionnaireHelper}" property="s2sQuestionnaireHelper"
		...
		<c:forEach var="narrative" items="${KualiForm.document.developmentProposalList[0].narratives}"
			varStatus="status">
			<c:if
		...
		<c:if
			...
			<c:if
				test="${KualiForm.proposalDevelopmentParameters['enableSummaryAttachmentsPanel'].value == 'Y'}">
		...
		<c:if test="${fn:length(KualiForm.document.developmentProposalList[0].propScienceKeywords) > 0}">
			<c:if
				test="${KualiForm.proposalDevelopmentParameters['enableSummaryKeywordsPanel'].value == 'Y'}">
		...
		<c:if
			test="${KualiForm.proposalDevelopmentParameters['enableSummaryDataValidationPanel'].value == 'Y'}">
			<kra:dataValidation auditActivated="${KualiForm.auditActivated}"
				...
				topTab="false" title="Validations" helpParameterNamespace="KC-PD"
				helpParameterDetailType="Document" helpParameterName="proposalDevelopmentDataValidation1HelpUrl">
				<p>You can activate a Validation check to determine any errors or incomplete information.
					The following Validations types will be determined:</p>
				<ul>
					<li>errors that prevent submission into routing</li>
					<li>warnings that serve as alerts to possible data issues but will not prevent submission
						into routing</li>
	...
	<div id="globalbuttons" class="globalbuttons">
		...
		<c:if test="${!empty KualiForm.editingMode['rejectProposal'] and not suppressRoutingControls}">
			<html:image src="${ConfigProperties.kra.externalizable.images.url}buttonsmall_reject.gif"
				styleClass="globalbuttons" property="methodToCall.reject" title="reject" alt="reject"
				onclick="resetScrollPosition();" tabindex="${tabindex}" />
		</c:if>
